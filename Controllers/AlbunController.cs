    ...
    public class AlbunController : ControllerBase
    {
        ...
        public async Task<ActionResult<APIResponse>> Search(string? nombreAlbun, string? nombreBandaCantante, int? IdGenero, string? IdPais) 
        {
            try
            {
                ...
                join varGenero in genero on varAlbun.IdGenre equals varGenero.IdMusicGenre
                select new {
                            varAlbun.IdAlbun,
                            varAlbun.AlbunName,
                            varAlbun.AlbunYear,
        ...
        public async Task<ActionResult<APIResponse>> CreateAlbun([FromBody] AlbunCreateDto createDto)
        {
            try
            {
                ...
                var genre = await _genreRepo.GetOne(g=>g.IdMusicGenre == modelo.IdGenre);
                if (genre == null) {
                ...
                modelo.IdAlbun =idx;
                modelo.PublishDate = DateTime.Now;
